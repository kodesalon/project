...
class MemberSelectResponseTest {
    ...
    void create_constructor_init_filed() {
        ...
        List<BoardSelectResponse> ownBoards =
                Collections.singletonList(new BoardSelectResponse(1L, "게시물 제목", "게시물 내용", LocalDateTime.now(), 1L, "alias", Collections.emptyList()));
        MultiBoardSelectResponse<BoardSelectResponse> multiBoardSelectResponse = new MultiBoardSelectResponse<>(ownBoards, 1);
        ...
        softly.then(memberSelectResponse.getOwnBoards()).isNotNull();
        softly.assertAll();
    }
}
