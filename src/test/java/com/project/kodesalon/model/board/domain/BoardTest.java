...
public class BoardTest {
    ...
    @Test
    @DisplayName("게시물의 제목, 내용을 수정한다.")
    void update() {
        board.updateTitleAndContent(new Title("update title"), new Content("update content"));

        softly.then(board.getTitle()).isEqualTo("update title");
        softly.then(board.getContent()).isEqualTo("update content");
        softly.assertAll();
    }
